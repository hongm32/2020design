...
说明：1、字段列表，可使用通配符*，表示所有字段
    ...
    SELECT * FROM books WHERE 数量<5 OR 类型='教材'
模糊查询：SELECT WHERE LIKE
      3、LIKE连接：使用通配符来代替字符：%表示0或任意多个，_表示1个，如
    SELECT * FROM books WHERE 作者 LIKE '李%'
TOP子句：
    SELECT TOP 2 * FROM [book]  # 表示选择前2条记录
    SELECT TOP 2 PERCENT * FROM [book]  # 表示选择前2%的记录
ORDER BY子句：
    SELECT TOP 2 * FROM [book] ORDER BY 书名 DESC  # 表示按书名排序，DESC表示降序
聚合查询：
    ...
    2、常用的聚合函数：
        ...
        SUM()    # 求和函数
        AVG()    # 求平均值函数，如
    SELECT TOP 4 ISBN,COUNT(*) AS [借阅数量] FROM [borrow] GROUP BY ISBN ORDER BY COUNT(*) DESC
多表查询：
    SELECT borrow.学号,student.姓名,borrow.ISBN,books.书名 FROM books,borrow,student WHERE borrow.学号=student.学号 AND borrow.ISBN=books.ISBN
"""
