...
def CmdChk():
    ...
    if v.get() == 0:
        tkinter.messagebox.showinfo('旅行小助手系统','请先选择数据表名。')
        return 0
    tree.destroy()
    vbar.destroy()
    conn.Open(DSN)
...
def callback0():
    if tkinter.messagebox.askokcancel("旅行小助手系统", "真心要退出？"):
        root.destroy()
...
def CmdSubmit():   # 提交
    ...
    for i in range(len(fieldsname)):
        if eval("va" + str(i) + ".get()").strip() == '':
            tkinter.messagebox.showwarning('旅行小助手系统', '[' + fieldsname[i] + ']不能为空。')
            e1[i].focus_set()
            return
...
def CmdDel():      # 删除记录（可以同时选中多行同时删除）
    ...
    if CrRecFstVolVal == []:   # 如果没有选中记录
        tkinter.messagebox.showinfo('旅行小助手系统', '请单击要删除的记录。')
        return
    if tkinter.messagebox.askokcancel('旅行小助手系统', '确实要删除“' + TblName + '”中' + FstFldName + '为' + str(CrRecFstVolVal) + '的记录吗？\n危险！此操作不可恢复！'):
        conn.Open(DSN)
...
def CmdMdf():      # 修改记录
    if v.get() == 0:
        ...
        return
    if CrRecFstVolVal == []:   # 如果没有选中记录
        tkinter.messagebox.showinfo('旅行小助手系统', '请单击要修改的记录。')
        return
    global tree, va0, va1, va2, va3, va4, va5, top, e1, rec1val, AddMdf
    AddMdf = 1
    root.withdraw()   # 隐藏窗体,防止交叉操作
    top=Toplevel(root)
...
def CmdQry():
    tkinter.messagebox.showinfo('旅行小助手系统', '“查询”功能期待你的加盟哦~~')

def CmdGrp():
    tkinter.messagebox.showinfo('旅行小助手系统', '“旅行组团”功能期待你的加盟哦~~')
...
# DSN = 'PROVIDER = Microsoft.Jet.OLEDB.4.0;DATA SOURCE = 旅行小助手.mdb'  # Access2003及以前
DSN = 'PROVIDER = Microsoft.ACE.OLEDB.12.0;DATA SOURCE = 旅行小助手.mdb'  # Access2007及以后

root = Tk()
root.title("旅行小助手系统")
size = '%dx%d+%d+%d' % (600, 340, root.winfo_screenwidth() / 2 - 440, root.winfo_screenheight() / 2 - 310)
...
rb1 = Radiobutton(root, text='成员信息管理', command=CmdChk, variable=v, value=1)
rb1.grid(row=0, column=0)
rb2 = Radiobutton(root, text='旅行线路管理', command=CmdChk, variable=v, value=2)
rb2.grid(row=0, column=1)
rb3 = Radiobutton(root, text='旅行记录管理', command=CmdChk, variable=v, value=3)
...
Cmd5=Button(root, text="旅行组团", command=CmdGrp, relief="solid", width=10)
